...
```sh
./packaging/appimage/ide/dist/GOWIN_EDA-v1.9.11.03-x86_64.AppImage
```
...
```sh
make -C packaging/appimage/ide validate
```

Create only the staged AppDir:

```sh
make -C packaging/appimage/ide appdir
```

Remove staged build files:

```sh
make -C packaging/appimage/ide clean
```

Remove staged files and the generated AppImage:

```sh
make -C packaging/appimage/ide distclean
```

See [RESEARCH_APPIMAGE.md](RESEARCH_APPIMAGE.md) for investigation details,
